...
// connTrackStateBitToState converts a Linux ConnTrackStateBit to a NF_CT_STATE bitmask.
func connTrackStateBitToState(stateBit linux.ConnTrackStateBit) uint32 {
	if stateBit < 0 {
		return linux.NF_CT_STATE_INVALID_BIT
	}
	if stateBit == linux.IP_CT_UNTRACKED {
		return linux.NF_CT_STATE_UNTRACKED_BIT
	}
	return (1 << ((stateBit)%linux.IP_CT_IS_REPLY + 1))
}
...
func (op *ctGet) evaluate(regs *registerSet, evalCtx opEvalCtx) {
	...
	switch op.key {
	case linux.NFT_CT_STATE:
		state := connTrackStateBitToState(ctInfo.StateBit)
...
func initCTGet(tab *Table, dreg uint8, attrs map[uint16]nlmsg.BytesView) (*ctGet, *syserr.AnnotatedError) {
	...
	dirU8, dirOK := AttrNetToHost[uint8](linux.NFTA_CT_DIRECTION, attrs)
	dir := linux.ConnTrackDir(dirU8)
	switch key {
	...
	case linux.NFT_CT_LABELS:
		len = linux.NF_CT_LABELS_MAX_SIZE
	case linux.NFT_CT_HELPER:
		len = linux.NF_CT_HELPER_NAME_LEN
	case linux.NFT_CT_SRC, linux.NFT_CT_DST:
	...
	len = (len + 3) & ^3 // round up to nearest multiple of 4.
