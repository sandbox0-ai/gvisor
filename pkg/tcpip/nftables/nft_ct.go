...
// ipctInfoToState converts an IPCTInfo to a NF_CT_STATE.
func ipctInfoToNFCTState(ctState linux.IPCTInfo) uint32 {
	// Check for stack.IPCTInvalidState or any -ve value.
	if ctState < 0 {
		return linux.NF_CT_STATE_INVALID_BIT
	}
	if ctState == linux.IP_CT_UNTRACKED {
		return linux.NF_CT_STATE_UNTRACKED_BIT
	}
	return 1 << (ctState%linux.IP_CT_IS_REPLY + 1)
}
...
func (op *ctGet) evaluate(regs *registerSet, evalCtx opEvalCtx) {
	...
	switch op.key {
	case linux.NFT_CT_STATE:
		state := ipctInfoToNFCTState(ctInfo.State)
