...
func newMasqOp(sregProtoMin, sregProtoMax int, flags uint32) (*masqOp, *syserr.AnnotatedError) {
	...
	// Ref: sizeof_field(struct nf_nat_range, min_proto.all).
	len := 2
...
func targetPortRangeForTCPAndUDP(originalSrcPort uint16) stack.PortOrIdentRange {
	...
	case originalSrcPort < 1024:
		return stack.PortOrIdentRange{Start: 1, Size: 1023}
	default:
...
// Matches Linux [net/netfilter/nft_masq.c]:nft_masq_eval()
func (m *masqOp) evaluate(regs *registerSet, evalCtx opEvalCtx) {
	...
	route := evalCtx.route
	if route == nil {
		regs.verdict = stack.NFVerdict{Code: VC(linux.NF_DROP)}
		return
	}

	// Get the network endpoint for the outgoing interface to find its primary address.
	netEP, err := evalCtx.nftState.stack.GetNetworkEndpoint(route.NICID(), route.NetProto())
	if err != nil {
		regs.verdict = stack.NFVerdict{Code: VC(linux.NF_DROP)}
		return
	}

	addressEP, ok := netEP.(stack.AddressableEndpoint)
	if !ok {
		regs.verdict = stack.NFVerdict{Code: VC(linux.NF_DROP)}
		return
	}
	...
	if !pkt.ConfigureNAT(ports, address, stack.SNAT, changePort, true /* changeAddress */) {
...
func (m *masqOp) Dump() ([]byte, *syserr.AnnotatedError) {
	...
	if m.sregProtoMinIdx >= 0 {
		msg.PutAttr(linux.NFTA_MASQ_REG_PROTO_MIN, nlmsg.PutU32(formatRegIdxForDump(int(m.sregProtoMinIdx))))
		msg.PutAttr(linux.NFTA_MASQ_REG_PROTO_MAX, nlmsg.PutU32(formatRegIdxForDump(int(m.sregProtoMaxIdx))))
	}
