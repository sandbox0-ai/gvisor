...
// ipv4AddrType determines the address type of the given IPv4 address.
// nicID represents the constraint interface ID.
// rt is the route to 'addr' (if one was found).
func (op *fib) ipv4AddrType(st *stack.Stack, addr tcpip.Address, nicID tcpip.NICID, rt *stack.Route) uint32 {
	switch {
...
// ipv6AddrType determines the address type of the given IPv6 address.
// nicID represents the constraint interface ID.
// rt is the route to 'addr' (if one was found).
func (op *fib) ipv6AddrType(st *stack.Stack, addr tcpip.Address, nicID tcpip.NICID, rt *stack.Route) uint32 {
	switch {
...
func (op *fib) getAddrType(netProto tcpip.NetworkProtocolNumber, st *stack.Stack, addr tcpip.Address, nicID tcpip.NICID, rt *stack.Route) uint32 {
	switch netProto {
	case header.IPv4ProtocolNumber:
		return op.ipv4AddrType(st, addr, nicID, rt)
	case header.IPv6ProtocolNumber:
		return op.ipv6AddrType(st, addr, nicID, rt)
	default:
		return uint32(linux.RTN_UNSPEC)
	}
}

// Ref: net/netfilter/nft_fib.c
...
func (op *fib) validatePktHeader(pkt *stack.PacketBuffer) bool {
...
func (op *fib) getSrcDstAddr(pkt *stack.PacketBuffer) (tcpip.Address, tcpip.Address, bool) {
	switch pkt.NetworkProtocolNumber {
	case header.IPv4ProtocolNumber:
		hdr := pkt.NetworkHeader().Slice()
		iph := header.IPv4(hdr)
...
// getOrFindRoute returns a route from the stack. If the route is found in the
// evalCtx, it is returned directly. Otherwise, the route is found using
// FindRoute.
func (op *fib) getOrFindRoute(evalCtx opEvalCtx, srcAddr, dstAddr tcpip.Address, netProto tcpip.NetworkProtocolNumber, nicID tcpip.NICID, dAddr bool) (rt *stack.Route, release func(), err tcpip.Error) {
...
func (op *fib) evaluateOIF(regs *registerSet, evalCtx opEvalCtx) {
	...
	if !op.validatePktHeader(evalCtx.pkt) {
		regs.verdict = stack.NFVerdict{Code: VC(linux.NFT_BREAK)}
		return
	}
	srcAddr, dstAddr, ok := op.getSrcDstAddr(pkt)
	if !ok {
		regs.verdict = stack.NFVerdict{Code: VC(linux.NFT_BREAK)}
		return
	}
	...
	case header.IPv4ProtocolNumber:
		hdr := pkt.NetworkHeader().Slice()
		iph := header.IPv4(hdr)
		if op.isV4Zeronet(iph.SourceAddress()) {
			if iph.DestinationAddress() == header.IPv4Broadcast || header.IsV4LinkLocalMulticastAddress(iph.DestinationAddress()) {
				op.storeResult(regs, pkt.NICID, st)
				return
			}
		}
		if srcAddr == header.IPv4Broadcast || header.IsV4MulticastAddress(srcAddr) || op.isV4Zeronet(srcAddr) {
			srcAddr = tcpip.Address{}
		}
	case header.IPv6ProtocolNumber:
		hdr := pkt.NetworkHeader().Slice()
		iph := header.IPv6(hdr)
		if op.ipv6SkipICMP(pkt, iph.SourceAddress(), iph.DestinationAddress()) {
			op.storeResult(regs, pkt.InputNICID, st)
			return
		}
	}
	...
	rt, release, err := op.getOrFindRoute(evalCtx, srcAddr, dstAddr, netProto, nicID, dAddr)
	defer release()
	if err != nil {
		return
	}

	addrType := op.getAddrType(netProto, st, addr, nicID, rt)
...
func (op *fib) ipv6SkipICMP(pkt *stack.PacketBuffer, saddr, daddr tcpip.Address) bool {
...
func (op *fib) isV4Zeronet(addr tcpip.Address) bool {
	slice := addr.AsSlice()
	return slice[0] == 0 && slice[1] == 0 && slice[2] == 0 && slice[3] == 0
}
...
func (op *fib) evaluateAddrType(regs *registerSet, evalCtx opEvalCtx) {
	...
	rt, release, _ := op.getOrFindRoute(evalCtx, srcAddr, dstAddr, netProto, nicID, dAddr)
	defer release()
