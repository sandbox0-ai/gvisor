...
type ConnTrackInfo struct {
	StateBit   linux.ConnTrackStateBit
	Direction  linux.ConnTrackDir
	SrcAddr    tcpip.Address
	DstAddr    tcpip.Address
	SrcPort    uint16
	DstPort    uint16
...
func (cn *conn) FillConnTrackInfo(opts ConnTrackInfoOpts, info *ConnTrackInfo) bool {
	stateBit := linux.ConnTrackStateBit(-1)
	if opts.FillState {
		cn.stateMu.RLock()
		...
		case tcpconntrack.ResultAlive, tcpconntrack.ResultReset,
			...
			if opts.UseReplyDir {
				stateBit = linux.IP_CT_ESTABLISHED_REPLY
			} else {
				stateBit = linux.IP_CT_ESTABLISHED
			}
