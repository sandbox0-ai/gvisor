...
func getExecveSeccheckInfo(t *Task, argv, env []string, executable *vfs.FileDescription, pathname string) (seccheck.FieldSet, *pb.ExecveInfo) {
	...
	if fields.Local.Contains(seccheck.FieldSentryExecveFdInfo) {
		info.Stdin = getExecveFdInfo(t, 0)
		info.Stdout = getExecveFdInfo(t, 1)
		info.Stderr = getExecveFdInfo(t, 2)
	}
...
func getExecveFdInfo(t *Task, fd int32) *pb.FdInfo {
	file := t.GetFile(fd)
	if file == nil {
		return nil
	}
	...
	if sops, ok := file.Impl().(socketImpl); ok {
