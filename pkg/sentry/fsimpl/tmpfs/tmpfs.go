...
type inode struct {
	...
	acl        atomic.Pointer[vfs.PosixACL] `state:".(*vfs.PosixACL)"`
...
func (i *inode) setStat(ctx context.Context, creds *auth.Credentials, opts *vfs.SetStatOptions) error {
	...
	mode := linux.FileMode(i.mode.Load())
	if err := vfs.CheckSetStat(ctx, creds, opts, mode, i.acl.Load(), auth.KUID(i.uid.Load()), auth.KGID(i.gid.Load())); err != nil {
		return err
	}
