...
// mustExec runs a command inside the systemd container and returns its
// output. It fatals on error.
func mustExec(ctx context.Context, t *testing.T, d *dockerutil.Container, args ...string) string {
	t.Helper()
...
func startService(ctx context.Context, t *testing.T, d *dockerutil.Container, unit string) {
	t.Helper()
	mustExec(ctx, t, d, "systemctl", "start", unit)
...
func stopService(ctx context.Context, t *testing.T, d *dockerutil.Container, unit string) {
	...
	mustExec(ctx, t, d, "systemctl", "stop", unit)
	out := mustExec(ctx, t, d, "systemctl", "show", "-p", "ActiveState", "--value", unit)
...
func TestSystemdBoot(t *testing.T) {
	...
	out := mustExec(ctx, t, d, "systemctl", "status", "systemd-journald.service", "--no-pager")
	...
	out = mustExec(ctx, t, d, "systemctl", "status", "systemd-logind.service", "--no-pager")
...
func TestSystemdSimpleDaemon(t *testing.T) {
	...
	mustExec(ctx, t, d, "systemctl", "enable", "test-daemon.service")
	startService(ctx, t, d, "test-daemon.service")

	wantDaemonActive := "active (running)"
	out := mustExec(ctx, t, d, "systemctl", "status", "test-daemon.service", "--no-pager")
	...
	// Get the main PID.
	pidStr := strings.TrimSpace(mustExec(ctx, t, d, "systemctl", "show", "-p", "MainPID", "--value", "test-daemon.service"))
	...
	out = mustExec(ctx, t, d, "journalctl", "-u", "test-daemon.service", "--no-pager")
	...
	mustExec(ctx, t, d, "kill", "-9", strconv.Itoa(pid2))
...
func TestSystemdNginx(t *testing.T) {
	...
	out := mustExec(ctx, t, d, "curl", "-fsSI", "http://127.0.0.1/")
	...
	mustExec(ctx, t, d, "systemctl", "reload", "nginx.service")
	out = mustExec(ctx, t, d, "curl", "-fsSI", "http://127.0.0.1/")
	if !strings.Contains(out, wantServerHeader) {
...
func TestSystemdApache(t *testing.T) {
	...
	// Graceful reload.
	mustExec(ctx, t, d, "systemctl", "reload", "apache2.service")
	out = mustExec(ctx, t, d, "curl", "-fsSI", "http://127.0.0.1/")
	if !strings.Contains(out, wantServerHeader) {
...
func TestSystemdPostgreSQL(t *testing.T) {
	...
	mustExec(ctx, t, d, "sudo", "-u", "postgres", "psql", "-v", "ON_ERROR_STOP=1", "-c", "CREATE TABLE gv (i int)")
	mustExec(ctx, t, d, "sudo", "-u", "postgres", "psql", "-v", "ON_ERROR_STOP=1", "-c", "INSERT INTO gv VALUES (42)")
	wantValue := "42"
	out := mustExec(ctx, t, d, "sudo", "-u", "postgres", "psql", "-tA", "-c", "SELECT i FROM gv")
...
func TestSystemdMariaDB(t *testing.T) {
	...
	mustExec(ctx, t, d, "mysql", "-e", "CREATE DATABASE gv; CREATE TABLE gv.t (i INT); INSERT INTO gv.t VALUES (42);")
	wantValue := "42"
	out := mustExec(ctx, t, d, "mysql", "-Ne", "SELECT i FROM gv.t")
	if !strings.Contains(out, wantValue) {
		t.Errorf("select output does not contain %q:\n%s", wantValue, out)
	}
...
func TestSystemdRedis(t *testing.T) {
	...
	wantPing := "PONG"
	if out := mustExec(ctx, t, d, "redis-cli", "ping"); !strings.Contains(out, wantPing) {
		t.Errorf("ping output does not contain %q: %s", wantPing, out)
	}
	wantValue := "42"
	mustExec(ctx, t, d, "redis-cli", "set", "gv", wantValue)
	if out := mustExec(ctx, t, d, "redis-cli", "get", "gv"); !strings.Contains(out, wantValue) {
		t.Errorf("get output does not contain %q: %s", wantValue, out)
	}

	// Force a background save to exercise the fork() path.
	mustExec(ctx, t, d, "redis-cli", "bgsave")
...
func TestSystemdDocker(t *testing.T) {
	...
	mustExec(ctx, t, d, "docker", "info")

	// Pull an image and verify it is stored.
	mustExec(ctx, t, d, "docker", "pull", alpineImageRef)
	wantImage := "mirror.gcr.io/library/alpine"
	out := mustExec(ctx, t, d, "docker", "image", "ls", alpineImageRef)
